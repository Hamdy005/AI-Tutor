...
function printAsPDF(title: string, contentHtml: string) {
  const htmlContent = `
    ...
    <body>
      <h1>${title}</h1>
      <div class="meta">Exported from Study Buddy on ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
      ${contentHtml}
      <hr>
  ...
  // Open the blob URL with noopener to separate the browsing contexts
